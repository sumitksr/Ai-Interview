...
/**
 * Signup OTP store — keyed by email (lowercased).
 * Stores { name, email, targetRole, hashedPassword, otp, expiresAt }
...
// Public — accepts { name, email, targetRole, password }
// Validates the data, hashes the password, sends OTP. No DB write.
export async function POST(req) {
  try {
    ...
    // ── Hash password up front so we don't store plaintext in memory ──────────
    ...
    await transporter.sendMail({
      ...
      to: normalizedEmail,
      subject: "Verify your AI Interview Platform email",
      html: `
        <div style="font-family:sans-serif;max-width:480px;margin:auto;background:#0a0f1c;padding:32px;border-radius:16px;border:1px solid #263246;">
          <h2 style="color:#2dd4bf;margin-top:0;">Verify your email 🚀</h2>
          <p style="color:#cbd6e4;">Hi ${name},</p>
          <p style="color:#cbd6e4;">You're almost there! Enter the OTP below to verify your email and complete your AI Interview Platform account setup. It expires in <strong>10 minutes</strong>.</p>
          ...
          <p style="color:#97a6ba;font-size:13px;">If you didn't create an AI Interview Platform account, you can safely ignore this email.</p>
        </div>
      `,
    });
